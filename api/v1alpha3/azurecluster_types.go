...
// For CAPZ controlplanes, this is currently the same as NodeResourceGroup.
func (c *AzureCluster) ControlPlaneResourceGroup() string {
	return c.NodeResourceGroup()
}

// NodeResourceGroup returns the cluster resource group.
func (c *AzureCluster) NodeResourceGroup() string {
...
func (c *AzureCluster) OutboundPool() string {
	return fmt.Sprintf("%s-%s", c.LoadBalancer(), "outboundBackendPool")
}

// LoadBalancer returns the node load balancer name.
func (c *AzureCluster) LoadBalancer() string {
