...
type Spec struct {
	...
	// NetworkPolicy used for building Kubernetes network. Possible values include: 'NetworkPolicyCalico', 'NetworkPolicyAzure'. Defaults to Azure.
...
func (s *Service) Get(ctx context.Context, spec interface{}) (interface{}, error) {
	...
	if !ok {
		return containerservice.ManagedCluster{}, errors.New("expected managed cluster specification")
	}
...
func (s *Service) Reconcile(ctx context.Context, spec interface{}) error {
	...
	// ****WARNING****
	// ****WARNING****
	// TODO(ace): probably don't do this.
	settings, err := auth.GetSettingsFromEnvironment()
	if err != nil {
		return fmt.Errorf("failed to parse azure environment settings: %v", err)
	}
	aadClientID, aadClientSecret := settings.Values[auth.ClientID], settings.Values[auth.ClientSecret]
	...
	if managedClusterSpec.NetworkPolicy != nil {
		properties.NetworkProfile.NetworkPolicy = containerservice.NetworkPolicy(*managedClusterSpec.NetworkPolicy)
	}
